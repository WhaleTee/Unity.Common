...
namespace WhaleTee.Runtime.Coroutine {
  /// <summary>
  ...
  public static class CoroutineTool {
    ...
    public static IEnumerator Lerp(
      float duration, Action<float> action, bool realTime = false, bool smooth = false,
      AnimationCurve curve = null, bool inverse = false
    ) {
      float time = 0;
      ...
      while (time < duration) {
        var delta = realTime ? Time.fixedDeltaTime : Time.deltaTime;
    ...
    /// <returns></returns>
    public static UnityEngine.Coroutine Lerp(MonoBehaviour obj, float duration, Action<float> action) {
