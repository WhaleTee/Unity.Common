...
namespace WhaleTee.Reactive.Runtime.MVVM.Editor
{
    ...
    {
        protected abstract SerializedProperty propertyName { get; set; }
        ...
        private void DrawPropertyNames()
        {
            ...
            {
                propertyName.stringValue = result == MVVMConstants.NONE ? null : result;
            ...
            var displayName = string.IsNullOrEmpty(propertyName.stringValue)
                ? MVVMConstants.NONE
            ...
            if (!IsValidPropertyName(propertyName.stringValue, viewModelType))
            {
                EditorGUILayout.HelpBox($"Property Name ({propertyName.stringValue}) not found in ViewModel: {viewModelType.Name}. Please choose correct property name.", MessageType.Warning);
            }
        }
